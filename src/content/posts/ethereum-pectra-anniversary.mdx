...
## 도입 — *"디지털 금 vs 글로벌 분산 컴퓨터"* 의 본질적 차이

자, 이더리움 강의를 시작합시다. 첫 번째 질문 — 이더리움은 왜 비트코인과 다른 framework 으로 분석해야 할까요?

이 질문은 그냥 단순한 *"코인 비교"* 가 아닙니다. 학술적으로 매우 중요한 분류 문제예요. 왜냐하면 — *"같은 자산군"* 으로 분류하면 같은 가치평가 도구를 써야 하고, *"다른 자산군"* 으로 분류하면 다른 도구가 필요하거든요. 이게 학자가 *"asset class taxonomy"* 라고 부르는 분야입니다.

비트코인에 대해서는 시리즈 1편 (#1-A~#1-J) 에서 — *"디지털 금"* framework 으로 분석했습니다. 21M 한도, ETF, 매크로 베타, NVT/MVRV — 모두 *"가치 저장소(store of value)"* 시각의 도구죠. 비트코인이 — Wei Dai b-money, Adam Back Hashcash, Nick Szabo Bit Gold 의 cypherpunk 계보 위에 있는, *"디지털 화폐 진화의 4단계"* 라는 framework.

그러나 이더리움은 — 출발이 다릅니다. Vitalik Buterin 의 2014년 백서 첫 문장이 — *"A Next Generation Smart Contract and Decentralized Application Platform"*. 즉, 이더리움은 *"화폐"* 가 아니라 *"플랫폼"* 으로 시작했어요. 이 차이가 — 가치평가 framework 을 완전히 다르게 만듭니다.

2026년 7월 현재 이더리움 시가총액은 약 2,790억 달러. BTC 시가총액의 약 18% 수준이지만 **자산 클래스 자체가 다릅니다**. 본 시리즈는 ETH 를 BTC 와 같은 잣대로 보지 않습니다.

비유로 풀어보겠습니다. Bankless 의 Mike Ipolito 는 2022년 글에서 **"BTC 가 금이라면 ETH 는 정유사(oil refinery)"** 라고 표현했습니다. 금은 — 가만히 있어도 가치를 보존하죠. 가치 저장 그 자체. 정유사는 — 매일 원유를 가공해서 가치를 만들어냅니다. 가치 *"창출"*. 그 가공 과정에서 발생하는 fee 가 — ETH 토큰의 가치 포착(value capture) 메커니즘이 됩니다.

이 비유가 학술적으로 정확한 이유가 있어요. 학자 Cong et al. (2021) RFS 논문 *"Tokenomics: Dynamic Adoption and Valuation"* 이 — 토큰의 *"이중 역할"* (사용 가치 + 투기 가치) 을 정량 모델링했습니다. 비트코인은 *"투기 가치"* 가 압도적이지만, 이더리움은 *"사용 가치"* 가 큰 비중. 이 학술 framework 위에서 — Mike Ipolito 의 비유가 정확하게 자리잡습니다.
