...
<Callout type="opposing-view" title="반대 시각 · 양자 buffer 5-10년 가설의 한계">
첫째, "5-10년 buffer" 추정은 현재 양자 진척도의 선형 외삽에 기반한다. Google Willow의 below-threshold 달성(2024.12)처럼 비선형적 돌파가 일어나면 buffer가 급격히 줄어들 수 있다. NSA·NIST는 "Harvest Now, Decrypt Later" 시나리오를 이미 경고하고 있어, 실제 위험은 Q-Day 도달 이전부터 시작된다는 분석이다.

둘째, BTC가 Soft fork로 양자 내성 알고리즘 전환에 도달하기까지 Taproot 사례(약 4년)와 같은 합의 시간이 필요하지만, BTC의 보수적 거버넌스 문화는 더 긴 시간을 요구할 수 있다. Dilithium·SPHINCS+ 채택에 대한 BIP 논의는 2026년 5월 현재 본격적으로 시작되지도 않았다.

셋째, 분실·도난된 약 350만 BTC와 사토시 본인의 약 100만 BTC 등 마이그레이션 불가능한 BTC가 양자 공격의 즉각적 표적이 된다. 이 BTC가 양자 공격으로 시장에 풀리면 가격에 미치는 충격은 통상 모델로 예측되지 않는다.

넷째, 서명 사이즈 38~780배 증가는 BTC의 블록 처리량을 근본적으로 제약한다. SegWit·Taproot도 1MB 블록 한계 안에서 작동했지만, Dilithium 서명은 블록 사이즈 확대 없이는 처리량 급감을 의미한다. 이는 BTC의 사용성 자체를 위협한다.

다섯째, NIST 표준 자체가 영구적이지 않다. CRYSTALS-Kyber에 대한 일부 측면 채널 공격(side-channel attack) 논문이 발표됐고, post-quantum 알고리즘이 양자 외 다른 공격 벡터에 취약할 수 있다는 점이 학계 우려다.

이 비판을 종합하면 양자 위협 대응이 "5-10년 buffer + Soft fork 전환"으로 정리되기에는 변수가 많고, BTC의 long-tail risk 중 가장 본질적이라는 학계 시각이 두텁다.
</Callout>
