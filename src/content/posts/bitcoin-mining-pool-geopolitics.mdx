...
비트코인은 "분산된 화폐" 라고 불립니다. 그러나 채굴 인프라를 보면 그림이 다릅니다 — 상위 5개 마이닝 풀이 전체 해시레이트의 80%+ 를 점유, 미국 단일 국가가 38%+ 점유. 이게 51% 공격 비용을 낮추거나 정치 리스크를 키우는가? 본 편은 그 trade-off 를 정량 분석합니다.
</Callout>
