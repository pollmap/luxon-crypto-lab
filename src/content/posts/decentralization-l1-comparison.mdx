...
**(1)** 같은 회사가 여러 entity 운영하는 case 미반영 (Coinbase + cbETH + Coinbase Custody = 같은 회사).
...
이 한계 때문에 Messari 등은 **Multi-dimensional Nakamoto** (5–8개 축 가중평균) 을 제안한다. 본 시리즈 #D5 trade-off 편에서 다시 다룬다.
</Callout>
...
탈중앙화는 단일 지표가 아니다. **어떤 공격 모델을 가정하느냐** 에 따라 우선 축이 달라진다. 다음 #D3 편에서 이 측정 framework 가 거버넌스 모델 (DAO vs Foundation vs 무명) 에 어떻게 적용되는지 다룬다.

본 글은 분석이며 매수/매도 추천이 아닙니다. L1 분산화 정량 비교의 학술적 framework 정리입니다.
