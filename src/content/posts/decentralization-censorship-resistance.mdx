...
2022년 8월 미국 재무부 OFAC 가 **Tornado Cash 스마트 컨트랙트 자체** 를 제재 명단에 추가했다. 사람도, 회사도 아닌 코드를 제재한 첫 case 였다. 2023년 개발자 Roman Storm 이 자금세탁 혐의로 기소됐고, 2024년 Coin Center 의 Van Loon v. Treasury 소송에서 Fifth Circuit 은 OFAC 결정을 무효화했다. 본 글은 이 4년의 사건을 검열저항이라는 framework 로 분석한다.
</Callout>

## 도입 — 코드를 제재할 수 있는가

검열저항 (censorship resistance) 은 가상자산 시스템의 핵심 가치 중 하나다. 그러나 이 가치가 실제로 시험되는 case 는 드물다 — 정부가 특정 거래나 사용자를 차단하라고 요구하는 경우다.
...
검열저항은 가상자산 옹호 측의 핵심 주장이지만, 비판도 명확하다. (a) 북한 Lazarus Group 이 Tornado Cash 로 $7B+ 자금세탁한 사실은 부인 불가능. 검열저항이 범죄 자금의 도구로 작동한 case 다. (b) 미국·EU 등의 시각에서는 "OFAC 제재 우회" 가 곧 정당성 약화 — 합법 시장 진입 비용을 키운다. (c) Bitcoin·Monero 의 강한 검열저항이 일반 사용자에게 일상적 효용을 주는지 의문. 즉 검열저항은 "0 또는 1" 이 아니라 비용/편익 trade-off 의 한 차원이며, 본 시리즈 #D5 편에서 다시 다룬다.
</Callout>
...
이 표의 의미는 **검열저항은 architectural + political + 법적 보호 의 결합**이라는 것이다. Tornado Cash case 는 법적 보호가 architectural 보호를 강화한 드문 사례.

다음 #D5 편에서 5편 시리즈 종합 — 분산화 vs 효율성 trade-off 가 어떻게 자산 선택의 framework 가 되는지를 다룬다.

본 글은 분석이며 매수/매도 추천이 아닙니다. 검열저항 case study 의 학술적 framework 정리입니다.
